...
namespace DFW
{
    namespace DRender
    {
        ...
        SharedPtr<TextureData> TextureLibrary::CreateTexture(DResource::ImageData const* const a_image_data)
        {
            ...
            SharedPtr<TextureData> const texture = MakeShared<TextureData>(texture_handle, a_image_data->width, a_image_data->height, BGFX_TEXTURE_NONE, uint8(0));
            texture->filepath = a_image_data->filepath;
            return texture;
        }
