...
template <> struct fmt::formatter<glm::vec2>
{
    ...
    format_context::iterator format(glm::vec2 const& a_vec2, format_context& a_ctx)
    {
        return presentation == 'f'
            ? fmt::format_to(a_ctx.out(), "({:.3f}, {:.3f})", a_vec2.x, a_vec2.y)
            : fmt::format_to(a_ctx.out(), "({:.3e}, {:.3e})", a_vec2.x, a_vec2.y);
    }
};
...
template <> struct fmt::formatter<glm::vec4>
{
    ...
    {
        return presentation == 'f'
            ? fmt::format_to(a_ctx.out(), "({:.3f}, {:.3f}, {:.3f}, {:.3f})", a_vec4.x, a_vec4.y, a_vec4.z, a_vec4.w)
            : fmt::format_to(a_ctx.out(), "({:.3e}, {:.3e}, {:.3e}, {:.3e})", a_vec4.x, a_vec4.y, a_vec4.z, a_vec4.w);
    }
};
...
template <> struct fmt::formatter<glm::quat>
{
    ...
    {
        return presentation == 'f'
            ? fmt::format_to(a_ctx.out(), "({:.3f}, {:.3f}, {:.3f}, {:.3f})", a_quat.x, a_quat.y, a_quat.z, a_quat.w)
            : fmt::format_to(a_ctx.out(), "({:.3e}, {:.3e}, {:.3e}, {:.3e})", a_quat.x, a_quat.y, a_quat.z, a_quat.w);
    }
};
