...
#pragma region Template Function Implementation
	...
	{
		auto handle = entity_manager_->GetEntity(a_entity_id);
		...
		else
		{
			...
			{
				ECSComponent* component_ptr(component_vectors_.at(typeid(T)).at(handle.GetID()));
				return ECSComponentHandle<T>(static_cast<T*>(component_ptr));
			}
		}
	}
	...
	ECSComponentManager::InternalHasComponents(EntityID a_entity_id) const
	{
		...
		auto entity = entity_manager_->GetEntity(a_entity_id);
